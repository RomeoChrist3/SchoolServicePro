...
async function initDatabase() {
  ...
  try {
    const tableInfoEtudiants = await executeQuery('DESCRIBE etudiants');
    if (tableInfoEtudiants) {
        if (!tableInfoEtudiants.some(col => col.Field === 'sexe')) {
        ...
        if (!tableInfoEtudiants.some(col => col.Field === 'matricule')) {
    ...
    const tableInfoProfesseurs = await executeQuery('DESCRIBE professeurs');
    if (tableInfoProfesseurs) {
        if (!tableInfoProfesseurs.some(col => col.Field === 'sexe')) {
        ...
        if (!tableInfoProfesseurs.some(col => col.Field === 'taux_horaire')) {
        ...
        if (!tableInfoProfesseurs.some(col => col.Field === 'heures_mensuelles_prevues')) {
    ...
    const tableInfoSettings = await executeQuery('DESCRIBE settings');
    ...
    const tableInfoNotes = await executeQuery('DESCRIBE notes');
    ...
    const tableInfoAbsences = await executeQuery('DESCRIBE absences');
    ...
    const tableInfoPaiements = await executeQuery('DESCRIBE paiements');
    ...
    const tableInfoMoratoires = await executeQuery('DESCRIBE moratoires');
    ...
    const tableInfoPointsCaisse = await executeQuery('DESCRIBE points_caisse');
    if (tableInfoPointsCaisse && !tableInfoPointsCaisse.some(col => col.Field === 'is_default')) {
    ...
    const tableInfoCaisse = await executeQuery('DESCRIBE caisse');
    if (tableInfoCaisse) {
        if (!tableInfoCaisse.some(col => col.Field === 'id_caisse')) {
            await executeQuery("ALTER TABLE caisse ADD COLUMN id_caisse INT NULL AFTER id");
        }
        if (!tableInfoCaisse.some(col => col.Field === 'code_caisse')) {
            await executeQuery("ALTER TABLE caisse ADD COLUMN code_caisse VARCHAR(50) NULL AFTER id_caisse");
        }
        if (!tableInfoCaisse.some(col => col.Field === 'id_paiement')) {
  ...
  try {
    const defaultPoints = await executeQuery('SELECT COUNT(*) as count FROM points_caisse');
